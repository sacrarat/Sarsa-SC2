...
class MassReaperBot(sc2.BotAI):
    ...
    async def on_step(self, iteration):
        ...
        if iteration == 0:
            self.base_top_left = 1 if self.start_location.y <= 31 else 0
        ...
        if len(self.current_unit_group) > 0:
            ...
            smart_action, x, y = self.splitAction(rl_action)
            ...
            if smart_action == IDLE:
                ...
                target = Point2((target[0], target[1]))
                for unit in self.current_unit_group:
    ...
    def retrieve_state(self):
        ...
        for enemy in enemy_units.not_structure:
            y = int(math.ceil((enemy.position.y + 1) / 32))
            x = int(math.ceil((enemy.position.x + 1) / 32))
        ...
        for unit in current_unit_group:
            y = int(math.ceil((unit.position.y + 1) / 32))
            x = int(math.ceil((unit.position.x + 1) / 32))
        ...
        state = np.append(state, hot_squares)
        state = np.append(state, green_squares)
